...
static int get_vlcs_running(raop_rtp_mirror_t *raop_rtp_mirror) {
    char *command = "pgrep vlc | wc -l";
    FILE* file = popen(command, "r");
    int vlcs_running = 0;
    fscanf(file, "%d", &vlcs_running);
    pclose(file);
    logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "%d vlcs running", vlcs_running);

    return vlcs_running >> 1;
}

static bool airplay_running(raop_rtp_mirror_t *raop_rtp_mirror) {
    char *command = "sudo netstat -ulp  | grep letsving | wc -l";
    FILE* file = popen(command, "r");
    int airplay_screening = 0;
    fscanf(file, "%d", &airplay_screening);
    pclose(file);
    logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "%d airplay running", airplay_screening);

    return airplay_screening > 0;
}
static void 
set_i3_end(raop_rtp_mirror_t *raop_rtp_mirror) {
	int screen_count = get_vlcs_running(raop_rtp_mirror);
	if (airplay_running(raop_rtp_mirror)) {
		screen_count++;
	}

	if (screen_count <= 1) {
		//ours will be first window in workspace 2
		logger_log(raop_rtp_mirror->logger, LOGGER_INFO, " 0 screen !!");
		system("i3-msg 'workspace 1'");
		//onle singly window to be split horizontally and in focus
	} else {
		//ours will be second window in workspace 2
		system("i3-msg 'layout toggle split'");
		//two window split horizontally, recent window focused to be split vertical
	}	
}

static void 
set_i3_start(raop_rtp_mirror_t *raop_rtp_mirror) {
	...
	if (screen_count == 0) {
		//ours will be first window in workspace 2
		system("i3-msg 'workspace2'");
		logger_log(raop_rtp_mirror->logger, LOGGER_INFO, " first one do whatever");
		//onle singly window to be split horizontally and in focus
	} else if (screen_count == 1) {
		//ours will be second window in workspace 2
		system("i3-msg 'workspace2; splith'");
		logger_log(raop_rtp_mirror->logger, LOGGER_INFO, " second one do whatever");
		//two window split horizontally, recent window focused to be split vertical
	} else if (screen_count  == 2) {
		//our will be third windows in workspace 2
		system("i3-msg 'workspace2; splitt'");
		//thre
	} else if (screen_count == 3) {
		//ours will be fourth window in workspace 2
		system("i3-msg 'workspace2; focus left; focus up; splitt'");
	} else {
		logger_log(raop_rtp_mirror->logger, LOGGER_INFO, " switching to last focused workspace to fullscreen");
...
raop_rtp_mirror_thread(void *arg)
{
	...
	FILE* log_file;
	log_file = fopen("/home/letsving/.screencast.log", "a");
    ...
    while (1) {
        ...
        if (!raop_rtp_mirror->running) {
            ...
            logger_log(raop_rtp_mirror->logger, LOGGER_ERR, "raop_rtp_mirror->running is no longer true");
  			fprintf(log_file, "%d-%02d-%02d %02d:%02d:%02d,aiplay\n", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
			fclose(log_file);
			set_i3_end(raop_rtp_mirror);
            break;
        }
        MUTEX_UNLOCK(raop_rtp_mirror->run_mutex);
                      ...
                      uint8_t show_client_FPS_data)
{
	...
	set_i3_start(raop_rtp_mirror);
	//printf ("asdfsafasdfds\n");

	if (airplay_running(raop_rtp_mirror))  {
    	logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "airplay is indeed running in start");
	} else {
    	logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "airplay is not runnin in starting");
	}
    ...
    *mirror_data_lport = raop_rtp_mirror->mirror_data_lport;
    ...
    MUTEX_UNLOCK(raop_rtp_mirror->run_mutex);
	if (airplay_running(raop_rtp_mirror))  {
    	logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "airplay is indeed running in end");
	} else {
    	logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "airplay is not runnin in end");
	}
}

...
static int
airplay_running(httpd_t *raop_rtp_mirror) {
...
httpd_thread(void *arg)
{
    ...
    while (1) {
        ...
        for (i=0; i<httpd->max_connections; i++) {
			...
			if (airplay_running(httpd))  {
				logger_log(httpd->logger, LOGGER_INFO, "airplay is indeed running in start");
			} else {
				logger_log(httpd->logger, LOGGER_INFO, "airplay is not runnin in starting");
			}
                httpd_remove_connection(httpd, connection);
                continue;
            }
